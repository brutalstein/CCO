...
| CLI package installs and runs cleanly | `npm pack` + isolated install, `cco --help`/`doctor`/`inventory`/`analyze`/`run` exercised against a real Claude Code install (see CHANGELOG "Fixed" entries) | PASS |
...
| Aggressive-mode pruning is backed by non-inferiority evidence at runtime | `packages/core/src/optimization/optimizer.ts`, `packages/core/src/quality/gate.ts` | **NOT WIRED UP.** `DefaultQualityGate`/`EvidenceIndex` exist and are unit-tested but every live call site (`cco run`, `cco task`, `cco benchmark`) passes `evidence: { records: [] }` unconditionally, so `PRUNE_NONINFERIOR_REDUNDANT` can never actually fire from a real session today. Do not claim aggressive mode is evidence-backed in practice until this is wired up. |

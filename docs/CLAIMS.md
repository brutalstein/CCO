...
| Cross-platform (Windows/macOS/Linux) build, test, package, and isolated artifact verification | `.github/workflows/ci.yml` runs all gates on `ubuntu-latest`, `windows-latest`, `macos-latest` | Last published commit: PASS for build/test; current release diff: **PENDING CI**. Only Windows has been manually exercised against a real `claude` binary. |

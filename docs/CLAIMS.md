...
| One live utility-edit smoke run had equal observed task success in both arms | `benchmarks/published/simple-utility-v1-run_56a34bbd3db70084/summary.json`: 1 task family, 2 trials/arm | PASS at smoke grade; not statistical non-inferiority and not promotion-eligible |
| Live task quality is statistically non-inferior vs native | — | **NOT SUPPORTED YET.** Requires at least exploratory-grade compatible evidence. |
...
| Aggressive-mode pruning requires exact compatible non-inferiority evidence | `packages/core/src/quality/evidence.ts`, `packages/core/src/profile/compiler.ts`; mismatch coverage in `packages/core/test/evidence-applicability.test.ts` and exact-candidate/legacy cases in `packages/core/test/profile-compiler.test.ts` | PASS for the fail-closed contract. Existing v1/smoke records remain readable but are intentionally promotion-ineligible; no claim is made that current live evidence authorizes aggressive pruning. |
| Unknown capability semantics cannot be treated as structural irrelevance | Independent certainty fields in `packages/core/src/types.ts`, real builder/compiler regression matrix in `packages/core/test/semantic-certainty.test.ts` | PASS |
| Inventory/repository identity changes when optimization-relevant live state or inspected manifest semantics change | `packages/core/src/inventory/service.ts`, `packages/core/src/repo/fingerprint.ts`; `inventory-service.test.ts`, `repo-fingerprint.test.ts` | PASS; content/state hashes contain no raw manifest bodies or absolute repository paths |
| Cross-platform (Windows/macOS/Linux) build, test, package, and isolated artifact verification | `.github/workflows/ci.yml` runs all gates on `ubuntu-latest`, `windows-latest`, `macos-latest` | **PASS** — release candidate `c05352a` passed [CI run 33031523306](https://github.com/brutalstein/CCO/actions/runs/33031523306). Only Windows has been manually exercised against a real `claude` binary. |
